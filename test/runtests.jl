using MessagePassingIPA: RigidTransformation, InvariantPointAttention, InvariantPointGate, transform, inverse_transform, compose, rigid_from_3points
...
@testset "MessagePassingIPA.jl" begin
    ...
    @testset "InvariantPointGate" begin
        n_dims_s = 32
        n_dims_z = 16
        ipg = InvariantPointGate(n_dims_s, n_dims_z)
        ...
        x3 = c .+ randn(Float32, 3, n_nodes)
        rigid1 = RigidTransformation(rigid_from_3points(x1, x2, x3)...)
        @test ipg(g, s, z, rigid1) isa Matrix{Float32}
        @test size(ipg(g, s, z, rigid1)) == (n_dims_s, n_nodes)
        ...
        x3 = R * x3 .+ t
        rigid2 = RigidTransformation(rigid_from_3points(x1, x2, x3)...)
        @test ipg(g, s, z, rigid1) ≈ ipg(g, s, z, rigid2)
    end
end
